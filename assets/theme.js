...
`;
  ...
  var ProductItem = class extends CustomHTMLElement {
    ...
    async _onColorSwatchChanged(event, target) {
      /* Code added by FieryMayo on product grid */
      ...
      if (target.hasAttribute("data-price")){
        ...
        let savings = "";
        var compare_price = (target.hasAttribute("data-compare-price") == true ) ? target.getAttribute("data-compare-price") : '';
        if (compare_price != '' && compare_price > price ) {
          
          if (window.themeVariables.settings.discountMode === "percentage") {
            savings = `${Math.round((compare_price - price) * 100 / compare_price)}%`;
          } else {
            savings = formatMoney(compare_at_price - price);
          }
        }else{

        }
        var parentContainer = target.closest("product-item");
        var priceContainer = target.closest("product-item").querySelector(".price-list");
        if(compare_price != ''){
          priceContainer.querySelector(".price.price--compare").innerHTML = formatMoney(compare_price,currencyFormat);

        }else {
          priceContainer.querySelector(".price.price--compare").innerHTML = '';
        }
        var compare_label = parentContainer.querySelector(".product-item__label-list").querySelector(".compare_tag");
        if(savings != ""){
          //console.log()
          if(compare_label){
            compare_label.classList.remove("hidden")
          	compare_label.innerHTML = window.themeVariables.strings.collectionDiscount.replace("@savings@", savings);
          }else {
            var listNode = "<span class = 'label label--highlight compare_tag'>"+`${window.themeVariables.strings.collectionDiscount.replace("@savings@", savings)}`+ '</span>';
          	parentContainer.querySelector(".product-item__label-list").innerHTML = parentContainer.querySelector(".product-item__label-list").innerHTML + listNode ;
          }
        }else{
          if(compare_label){
        	compare_label.classList.add("hidden")
          }
        }
        
        priceContainer.querySelector(".price.normal").innerHTML = formatMoney(price,currencyFormat);

        //console.log(savings,compare_price,price)
      }
      if(target.hasAttribute("data-variant-size") && target.hasAttribute("data-base-url")){
        var parentCntainer = target.closest("product-item");
        var vsizes = target.getAttribute("data-variant-size");
        var base_ur = target.getAttribute("data-base-url");
        var base_main = "";
        console.log(vsizes)
        vsizes = vsizes.split(",");
      	//console.log(base_ur,sizes[0])
        for (let i=0; i< vsizes.length;i++) {
          var va = vsizes[i].replace("`","").split(":");
          var id = va[1];
          var title = va[0];
          if(id  && title){
            //var btn_n = btn_html.replace('${id}',va[1]);
             var btn_html = `<button type = "button" class = "single-option-button variant-btn " onclick = '`+base_ur+`${id}";' <span  class = "bacg"> ${title} </span></button>`
             base_main += btn_html;
          }

        }
        parentCntainer.querySelector(".size_variants").innerHTML = base_main;
      }
